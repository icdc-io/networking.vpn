...
const ClientConnectionDevices = ({ t, history }) => {
    ...
    useEffect(() => {
        if (account && devicesIds) {
            ws.current = new WebSocket(`ws${protocol}://${locationUrl.replace("api", "wireguard-wss")}/ws/wireguard_manager/stats?${devicesIds}`, ['actioncable-v1-json', window.insights.getToken(), account, role]);
            ws.current.onopen = () => {
                const subscribe_msg = {
                    command: 'subscribe',
    ...
    const addPopup = (content, header) => {
        ...
        return (
            <div className="flex">
                ...
                &ensp;
                {cellContent && (
                    <Popup on="click" pinned trigger={<div className="popup-dots">...</div>} inverted className="vpn">
                        {popupContent}
                    </Popup>
                )}
            </div>
    ...
    const displayTableData = vpnClientConnectionDevicesData.slice(pageViseted, pageViseted + totalPaginationPages).map((item, key) => <Table.Row key={key}>{tableCells(item)}</Table.Row>);
