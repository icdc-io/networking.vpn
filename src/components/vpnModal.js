...
const VpnModal = ({
  ...
  const nextIp =
    managementName === "vpnDevices"
      ? { ip: next_ip }
      : getIp(addr, vpnNatMapping, "vpn_ip", natSubnetValue);
  ...
  const onSubmit = (values) => {
    ...
    dispatch(
      prepPayloadForSubmitingAndSubmitFunction(
        messageText === "creatingConfig" ? values.id : id,
        values,
      ),
    );
    handleClose();
        ...
        <Modal size="tiny" open={open} onClose={handleClose} closeIcon>
          <Modal.Header
            content={
              openConfigs
                ? t("configs")
                : t(editContentMessage || addContentMessage)
            }
          />
          <Modal.Content style={{ paddingTop: "0" }}>
            {openConfigs ? (
              ...
              <VpnForm
                handleClose={handleClose}
                onSubmit={onSubmit}
                initialValues={edit || privateKey ? values : nextIp}
                fieldNames={formFields}
                edit={edit}
                pencil={pencil}
